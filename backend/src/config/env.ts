...
export function validateEnvironmentVariables() {
  ...
  const WHATSAPP_DEV_MODE = validateEnvVar(
    "WHATSAPP_DEV_MODE",
    process.env.WHATSAPP_DEV_MODE,
  );
  const WHATSAPP_COOLDOWN_SECONDS = validateEnvVar(
    "WHATSAPP_COOLDOWN_SECONDS",
    process.env.WHATSAPP_COOLDOWN_SECONDS,
  );
  const STORE_URL = validateEnvVar("STORE_URL", process.env.STORE_URL);
  const ADMINISTRATIVE_PANEL_URL = validateEnvVar(
  ...
  const RESEND_API_KEY = validateEnvVar(
    "RESEND_API_KEY",
    process.env.RESEND_API_KEY,
  );
  const RESEND_FROM = validateEnvVar("RESEND_FROM", process.env.RESEND_FROM);
  const MINIO_ENDPOINT = validateEnvVar(
    "MINIO_ENDPOINT",
    process.env.MINIO_ENDPOINT,
  );
  const MINIO_PORT = validateEnvVar("MINIO_PORT", process.env.MINIO_PORT);
  const MINIO_USE_SSL = validateEnvVar(
    "MINIO_USE_SSL",
    process.env.MINIO_USE_SSL,
  );
  const MINIO_ACCESS_KEY = validateEnvVar(
    "MINIO_ACCESS_KEY",
    process.env.MINIO_ACCESS_KEY,
  );
  const MINIO_SECRET_KEY = validateEnvVar(
    "MINIO_SECRET_KEY",
