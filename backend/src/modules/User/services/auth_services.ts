...
class AuthServices {
  ...
  async newUser(req: Request, res: Response) {
    ...
    if (Number(role_id) === 1) {
      ...
      const created: any[] =
        await prisma.$queryRaw`SELECT id, email, name, role, phone, profile_image, created_at, updated_at FROM "Admin" WHERE email = ${email} LIMIT 1`;
      user = created[0];
    } else {
  ...
  async getUsers(req: Request, res: Response) {
    ...
    if (typeQ === "admin") {
      try {
        ...
        if (searchQ) {
          countRows =
            await prisma.$queryRaw`SELECT COUNT(*)::int AS count FROM "Admin" WHERE name ILIKE ${pattern} OR email ILIKE ${pattern}`;
          rows =
            await prisma.$queryRaw`SELECT id, name, email, role, phone, is_active FROM "Admin" WHERE name ILIKE ${pattern} OR email ILIKE ${pattern} ORDER BY created_at DESC LIMIT ${limitQ} OFFSET ${(pageQ - 1) * limitQ}`;
        } else {
          countRows =
            await prisma.$queryRaw`SELECT COUNT(*)::int AS count FROM "Admin"`;
          rows =
            await prisma.$queryRaw`SELECT id, name, email, role, phone, is_active FROM "Admin" ORDER BY created_at DESC LIMIT ${limitQ} OFFSET ${(pageQ - 1) * limitQ}`;
        }
  ...
  async disableUser(req: Request, res: Response) {
    ...
    if (type === "admin") {
      const exists: any[] =
        await prisma.$queryRaw`SELECT id FROM "Admin" WHERE id = ${Number(id)} LIMIT 1`;
      if (!exists?.[0])
        return res.status(404).json({ ok: false, error: "user_not_found" });
  ...
  async changePasswordAdmin(req: Request, res: Response) {
    try {
      ...
      const rows: any[] =
        await prisma.$queryRaw`SELECT id, password FROM "Admin" WHERE id = ${Number(claim.sub || claim.id)} LIMIT 1`;
      const admin = rows[0];
      if (!admin)
        return res.status(404).json({ ok: false, error: "user_not_found" });
