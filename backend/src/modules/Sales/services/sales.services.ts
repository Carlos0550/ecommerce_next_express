...
class SalesServices {
  async saveSale(request: SaleRequest): Promise<string | true> {
    ...
    const primaryPaymentMethod =
      (request.payment_methods?.[0]?.method) || payment_method;
      ...
      void (async () => {
        try {
          ...
          if (
            ...
            logger.warn("No recipient configured for sale email");
          }
        } catch (err) {
          logger.error("sale_email_send_failed", {
  ...
  async updateSale(id: string, request: SaleRequest) {
    ...
    if (!existing) throw new NotFoundError("Venta no encontrada", "sale_not_found");
    const isManual = !!request.loadedManually;
    const manualItems = Array.isArray(request.manualProducts)
      ? request.manualProducts
      : [];
  ...
  async getSales({
    ...
    const [total, sales, totalSalesByDate] = await Promise.all([
      prisma.sales.count({ where: where as any }),
...
export const _unusedAppErr = AppError;