...
export function useListFaqsAdmin(page: number = 1, limit: number = 50) {
  ...
  return useQuery<
    { ok: boolean; items: FAQ[]; page: number; total: number },
    Error
  >({
    ...
    queryFn: async () => {
      ...
      const res = await fetch(`${baseUrl}/faqs/admin?${qs}`, {
...
export function useCreateFaq() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      notifications.show({ message: "FAQ creada", color: "green" });
    },
    onError: (e: Error) => {
      notifications.show({
        message: e?.message || "Error creando FAQ",
        color: "red",
      });
    },
  });
...
export function useUpdateFaq() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["faqs_admin"] });
      notifications.show({ message: "FAQ actualizada", color: "green" });
    },
    onError: (e: Error) => {
      notifications.show({
        message: e?.message || "Error actualizando FAQ",
        color: "red",
      });
    },
  });
...
export function useDeleteFaq() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["faqs_admin"] });
      notifications.show({ message: "FAQ eliminada", color: "green" });
    },
    onError: (e: Error) => {
      notifications.show({
        message: e?.message || "Error eliminando FAQ",
        color: "red",
      });
    },
  });
